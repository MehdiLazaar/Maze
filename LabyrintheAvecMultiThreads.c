...
void *trouverSortieMultiThread(void *params) {
    ...
    if (x > 0 && (m[x-1][y] == CASE_LIBRE || m[x-1][y] == SORTIE) && m[x-1][y] != CASE_VISITEE && m[x-1][y] != MUR) {
        ...
        pthread_create(&threads[nombrethreads], NULL, trouverSortieMultiThread, &ligneAvant);
        nombrethreads++;
    }
    ...
    if (x < LIGNES-1 && (m[x+1][y] == CASE_LIBRE || m[x+1][y] == SORTIE) && m[x+1][y] != CASE_VISITEE && m[x+1][y] != MUR) {
        ...
        pthread_create(&threads[nombrethreads], NULL, trouverSortieMultiThread, &ligneApres);
        nombrethreads++;
    }
    ...
    if (y > 0 && (m[x][y-1] == CASE_LIBRE || m[x][y-1] == SORTIE) && m[x][y-1] != CASE_VISITEE && m[x][y-1] != MUR) {
        ...
        pthread_create(&threads[nombrethreads], NULL, trouverSortieMultiThread, &colAvant);
        nombrethreads++;
    }
    ...
    if (y < COLONNES-1 && (m[x][y+1] == CASE_LIBRE || m[x][y+1] == SORTIE) && m[x][y+1] != CASE_VISITEE && m[x][y+1] != MUR) {
        ...
        pthread_create(&threads[nombrethreads], NULL, trouverSortieMultiThread, &colApres);
        nombrethreads++;
    }
...
int main(){
    ...
    struct Coordonnees coord = {argentree[0], argentree[1], matrice};
    pthread_create(&thread, NULL, trouverSortieMultiThread, &coord);

    //On attend la fin de l'execution du thread
